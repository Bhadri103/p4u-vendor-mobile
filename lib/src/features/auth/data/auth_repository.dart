...
Map<String, dynamic> _authPayloadFromPhoneExchange(
  ...
  for (var depth = 0; depth < 6; depth++) {
    if (_hasAccessToken(payload)) return payload;
    final nested = apiObject(payload['auth'] ??
        payload['session'] ??
        payload['tokens'] ??
        payload['data'] ??
        payload['result']);
    if (nested == null || identical(nested, payload)) break;
    payload = nested;
  }
  return payload;
}
  ...
  Future<VendorUser> signInWithFirebaseIdToken(String firebaseIdToken) async {
    ...
    if (!await apiSession.hasToken()) {
      throw const ApiException(
          'Login was verified but no vendor session was returned. Please try again.');
    }
    await _registerPush();
