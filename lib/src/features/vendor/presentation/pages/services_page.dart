...
class _ServicesPageState extends ConsumerState<ServicesPage> {
            ...
            child: ListView(
              ...
              children: [
                VendorPageIntro(
                  ...
                  title: '${items.length} service offerings',
                  subtitle:
                      'Shape your service menu, availability, pricing and reach.',
  ...
  Future<void> _toggle(Map<String, dynamic> s) async {
    ...
    if (pending) {
      _snack(
          'This listing is pending admin approval and cannot be activated yet.');
      return;
    }
    try {
      await ref
...
class _ServiceFormSheetState extends ConsumerState<_ServiceFormSheet> {
  ...
  void _onSelectSubcategory(
      String serviceId, List<Map<String, dynamic>> catalog) {
    final match =
  ...
  Future<void> _pickImage() async {
    ...
    try {
      final url = await pickVendorImage(ref, widget.vendorId, 'services');
        ...
        child: Column(
          ...
          children: [
            ...
            Row(children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(
                      _uploading ? 'Uploading...' : 'Upload Service Image'),
                ),
              ),
            ...
            catalog.whenUi(
              ...
              data: (items) {
                final options = items
                    ...
                    .toList();
                final valid =
                    options.any((c) => c['id']?.toString() == _serviceId);
            ...
            _YesNoField(
              ...
              value: _availability,
              onChanged:
                  _pending ? null : (v) => setState(() => _availability = v),
            ),
            _YesNoField(
              label: 'Trending',
              value: _trending,
      ...
      child: url.isEmpty
          ? const Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.image_outlined, color: AppColors.slate),
                  SizedBox(height: 6),
                  Text('No image', style: TextStyle(color: AppColors.muted)),
                ],
              ),
            )
          : Image.network(url,
