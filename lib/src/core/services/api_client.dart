...
class ApiClient {
  ...
  // P4U_API_BASE_URL is the documented public flag. Keep API_BASE_URL as a
  // compatibility fallback for existing build pipelines.
  ...
  Future<void> _refreshAuth() async {
    ...
    try {
      ...
      // A rejected refresh token is terminal. Clear it so the splash gate can
      // return to login instead of retrying a dead session forever.
  ...
  Future<Map<String, dynamic>> _decodeResponse(
    ...
    if (data['success'] == true && data.containsKey('data')) {
      ...
      // getJson exposes a map. A successful null payload (for example a vendor
      // with no food restaurant) maps to an empty object, not the envelope.
